...
!   Common name:      CH4O-2014
!
!   References: E. Gonzalez-Lavado, J.C. Corchado and J. Espinosa-Garcia
!                     J.Chem. Phys., Vol. 140, p. 064310, 2014.  
!
!   Notes:
!
!     SUBROUTINE POT
...
COMMON/USROCM/ PENGYGS,PENGYES(ISURF),PENGYIJ(JSURF),DGSCART(NATOM,3),DESCART(NATOM,3,ISURF),DIJCART(NATOM,3,JSURF)C
COMMON/USRICM/ CART(NATOM,3),ANUZERO,NULBL(NATOM),NFLAG(20),NASURF(ISURF+1,ISURF+1),NDER
COMMON /POTCM/ nnc,nnb,nnh(4),r0ch,d1ch,d3ch,a1ch,b1ch,c1ch,r0hh,d1hh,d3hh,ahh,r0cb,d1cb,d3cb,acb,a3s,b3s,aphi,bphi,cphi,atheta,btheta,ctheta,fch3,hch3,fkinf,ak,bk,aa1,aa2,aa3,aa4,a1s,b1s,a2s,b2s,vdwe, vdwr, vdwc1, vdwc2, vdt1, vdt2, vdtr 
...
call cartou
call carttor
! changing units from angstrom to bohr and initialize
do i = 1, 18
    q(i)    = r(i)
!   q(I) = R(I)*0.52918d0
    pdot(i) = 0.D0
...
call ipbend(vip)
! calculate van der Waals potential (new)
call vander(evdw)
! total potential energy is vstr+vop+vip
en     = vstr+vop+vip+evdw
! 0.03812 conversion factor from 10(5) j/mol to hartrees
en     = en*0.03812D0
engygs = en
call eunitzero
if( nder .ne. 0 )then
    ! 0.0201723 conversion factor from 10(5)j/mol/A to hartrees/bohr
    do i = 1, 18
...
subroutine refangles
    ...
    COMMON /POTCM/ nnc,nnb,nnh(4),
     ...
     +               a1s,b1s,a2s,b2s,
     +               vdwe, vdwr, vdwc1, vdwc2, vdt1, vdt2, vdtr 
    common /angles/  theta0(4,4),dtheta0(4,4,4)
    common /bonds/   rcb,rch(4),rbh(4)
    common /coords/  tcb(3),tch(4,3),tbh(4,3)
    common /delta1/  fdelta(4),hdelta(4)
...
!  calculate reference angles
       theta0(1,2)=tau+(tau-halfpi)*(sphi(1)*sphi(2)-1.0d0)+(tau-twopi/3.0d0)*(stheta(3)*stheta(4)-1.0d0)
       theta0(1,3)=tau+(tau-halfpi)*(sphi(1)*sphi(3)-1.0d0)+(tau-twopi/3.0d0)*(stheta(2)*stheta(4)-1.0d0)
       theta0(1,4)=tau+(tau-halfpi)*(sphi(1)*sphi(4)-1.0d0)+(tau-twopi/3.0d0)*(stheta(2)*stheta(3)-1.0d0)
       theta0(2,3)=tau+(tau-halfpi)*(sphi(2)*sphi(3)-1.0d0)+(tau-twopi/3.0d0)*(stheta(1)*stheta(4)-1.0d0)
       theta0(2,4)=tau+(tau-halfpi)*(sphi(2)*sphi(4)-1.0d0)+(tau-twopi/3.0d0)*(stheta(1)*stheta(3)-1.0d0)
       theta0(3,4)=tau+(tau-halfpi)*(sphi(3)*sphi(4)-1.0d0)+(tau-twopi/3.0d0)*(stheta(1)*stheta(2)-1.0d0)
! calculate the derivatives of theta0(i,j) in terms of rch(k) quantity calulated is dtheta0(i,j,k)
...
! subroutine to calculate leps-type stretching potential and its derivatives
subroutine stretch(vstr)
    ...
    COMMON /POTCM/ nnc,nnb,nnh(4),
     ...
     common /qpdot_pl/   q(150),pdot(150)
     common /switch1/ sphi(4),dsphi(4),stheta(4),dstheta(4)
     dimension vqch(4),vjch(4),vqbh(4),vjbh(4),vq(4),vj(4),achdc(3),achdh(4,3)
     ...
     !  calculate singlet: e1, triplet: e3 energies and vq and vj terms for each bond
...
c************************************************************************
c
       subroutine vander(evdw)
c
c  subroutine to calculate van der waals potential 
c
       implicit double precision (a-h,o-z)
      CHARACTER*75 REF(5)
C
...
c
       rav=(rch(1)+rch(2)+rch(3)+rch(4))/4.0d0
c
       p1vdw = vdwc1*(vdwr/rcb)**6.d0 
       p2vdw = vdwc2*exp(-12.d0*(rcb/vdwr))
       evdw1 = vdwe/0.03812d0/627.51 * (p1vdw+p2vdw)

       devdw = vdwe/0.03812d0/627.51 *
     *        ( -6*p1vdw/rcb - 12*p2vdw/vdwr)

c
c      tvdw = 1.d0+vdt1*tanh(vdt2*(rav-vdtr))
c
       arga=(vdt2*(rav-vdtr))
       if(arga.lt.19.0d0)then
         tvdw = 1.d0+vdt1*tanh(arga)
         dtvdw = vdt1*vdt2*(1/cosh(arga))**2.d0
       else
         tvdw=1.d0+vdt1
         dtvdw=0.0d0
       endif
...
c derivadas:
c
c derivada de evdw1 por tvdw sin derivar
c

       do ind = 1, 3
          pdot(nc(ind)) = pdot(nc(ind)) + tcb(ind)*devdw/rcb * tvdw
          pdot(nhb(ind)) = pdot(nhb(ind)) - tcb(ind)*devdw/rcb * tvdw
       enddo
c
c derivada de tvdw por evdw1 sin derivar
c derivada de tvdw = dtvdw * derivada de rav
c
       do ind = 1, 3
          do i = 1, 4
             pdot(nc(ind)) = pdot(nc(ind)) + evdw1 * dtvdw * 
     *       (1.d0/4.d0) * (q(nc(ind))-q(nh(i,ind)))/rch(i)
             pdot(nh(i,ind)) = pdot(nh(i,ind)) + evdw1 * dtvdw * 
     *       (1.d0/4.d0) * (q(nh(i,ind))-q(nc(ind)))/rch(i)
          enddo
       enddo
c
       return
...
C
       ...
       DATA a1ch    /   1.57025d0/
       DATA b1ch    /   0.13527d0/
       DATA c1ch    /  22.30400d0/
       DATA r0hh    /   0.97060d0/
       DATA d1hh    / 130.830d0/
       DATA d3hh    /  52.07464d0/
       DATA ahh     /   2.1005d0/
       DATA r0cb    /   1.487320/
       DATA d1cb    /  65.04919d0/
       DATA d3cb    /  19.98567d0/
       DATA acb     /   1.259d0/
       ...
       DATA b1s     /  -4.669630d0/
       DATA a2s     /   1.014740d0/
       DATA b2s     / -12.363800d0/
       DATA vdwe     /   0.100000d0/
       DATA vdwr     /   3.713100d0/
       DATA vdwc1    /  -6.700000d0/
       DATA vdwc2    /  22.500000d0/
       DATA vdt1    /   1.000000d0/
       DATA vdt2    /   9.000000d0/
       DATA vdtr    /   1.154807d0/
C
       END
