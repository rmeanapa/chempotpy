...
! Common name:      CH4OH
! Notes:
! References:  J. Espinosa-Garcia, J. C. Corchado, J. Chem. Phys.,
!              Vol. 112, p. 5731, 2000.
! Calling conventions:  Polyrate interface
...
common /addh2o/ded(3),angh2o(4),fkh2o(4),dkdr(4),dkdx(4,3),dedo(3),dedhi(3),dedho(3)
call cartou   
call carttor  
...
vstr  = vstr+vno
! partial derivatives
! derivative of OH morse term with respect to dt:
deddt = 2.0d0 * ahh * d1hh * (1.0d0-expterm) *expterm
! chain rule: derivatives with respect to cartesian coordinates.
de = deddt / rno
       do i=1,3
          ded(i) = de * tno(i)
       enddo
! now we need the derivative of ach:
       do ind=1,3
         ...
         enddo
       enddo
       dumqcb=-acb*((d1cb+d3cb)*exp(-2.0d0*acb*(rcb-r0cb))-(d1cb-d3cb)*exp(-acb*(rcb-r0cb)))/rcb
...
!  adding the derv of D3cb wrt r
         addd3 = dd3cb*(exp(-2.0d0*acb*(rcb-r0cb))+2.0d0*exp(-acb*(rcb-r0cb)))
         addd3 = addd3/4.d0 * 0.5d0 / rcb
         dumqbh = dumqbh + addd3
         factj=0.5d0/vj(i)
         dumjcb=-acb*((d1cb-d3cb)*exp(-2.0d0*acb*(rcb-r0cb))-(d1cb+d3cb)*exp(-acb*(rcb-r0cb)))*factj/rcb
         dumjbh=-ahh*((d1hh-d3hh)*exp(-2.0d0*ahh*(rbh(i)-r0hh))-(d1hh+d3hh)*exp(-ahh*(rbh(i)-r0hh)))*factj/rbh(i)
!  adding derv of D3cb wrt r
         addd3 = dd3cb*(exp(-2.0d0*acb*(rcb-r0cb))+2.0d0*exp(-acb*(rcb-r0cb)))
...
!  dv(i)/dh(j)
     ...
     *            -(vjbh(i)-vjch(i))*dumjhj
         ...
         enddo
       enddo
! add derivatives of OH morse term
       do ind=1,3
...
subroutine ipbend( vip )
    ...
    COMMON /POTCM/ nnc,nnb,nnh(5),nno,
     ...
     do i=1,3
         do j=i+1,4
           costh(i,j) = tch(i,1)*tch(j,1)+tch(i,2)*tch(j,2)+tch(i,3)*tch(j,3)
