...
!   Zero of energy: 
!      The classical potential energy is set equal to zero for the A
...
600   FORMAT(/,1X,'*****','Potential Energy Surface',1X,'*****',
     *      //,1X,T5,'Al + LC potential energy surface',
     *      //,1X,T5,'Parameters:',
     *        /,1X,T5,'Bond', T46, 'AI-L', T58, 'L-C', T69, 'C-AI',
     ...
     *        T44, F10.5, T55, F10.5, T66, F10.5)
610   FORMAT(/,1X,T5,'Parameters for the auxiliary barrier term ',
     *       '(a.u.)',/,1X,T20,'A = ',F15.8,T45,'alpha = ',T53,F15.8,
     *     //,1X,'*****')
...
6000  FORMAT(/,1X,'In the potential routine: T, RAD = ',1P,2E15.7,
...
C   Initialize the flags and the I/O unit numbers for the potential
         ...
         DATA NDUM /21*0/
C
C   Initialize the eLEPS potential parameters; the energy parameters are in
C   kcal/mol, and the lengths are in Angstroms.
         DATA DE/ 105.5D0, 105.5D0, 84.4D0/
         DATA RE/ 1.09D0, 1.09D0, 1.54D0/
         DATA BETA/ 1.78D0, 1.78D0, 1.94D0/
         DATA Z/ 0.2D0, 0.2D0, 0.1D0/
C   Initialize the potential parameters for the auxiliary barrier term; 
C   the parameters are in Hartree atomic units.
