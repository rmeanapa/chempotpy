...
!=======================================================================
!     The ^2A prime PES for C2N system at the MC-PDFT/AVTZ level.
!=======================================================================
!=======================================================================
...
!-----------------------------------------------------------------------
subroutine c2n_2a_prime(rb,path,Vpes)
    implicit none
...
subroutine nnfit_a(ndim,ntot,r0,path,vx)
      ...
      file_path1 = trim(path)//"/C2N/c2n_2a_prime_para_a.txt"
      if (init.eq.0) then
         init=1
         fid=123456
      ...
      integer,save :: fid
      if (ndim.ne.s0) stop "ndim .ne. s0"
      file_path1 = trim(path)//"/C2N/c2n_2a_prime_para_b.txt"
      if (init.eq.0) then
         init=1
         fid=123456
         open(fid,file=file_path1,status='old',action='read')
      ...
      integer,save :: fid
      if (ndim.ne.s0) stop "ndim .ne. s0"
      file_path1 = trim(path)//"/C2N/c2n_2a_prime_para_c.txt"
      if (init.eq.0) then
         init=1
         fid=123456
         open(fid,file=file_path1,status='old',action='read')
