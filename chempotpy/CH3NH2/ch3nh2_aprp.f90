...
real(8) ::, intent(in) :: x(natoms,3)
real(8) ::, intent(out) :: p(nstates), g(nstates,natoms,3)
real(8) ::, intent(out) :: d(nstates,nstates,natoms,3)

real(8) :: :: xx(3,natoms), uu(nstates,nstates)
real(8) :: :: guu(3,natoms,nstates,nstates)
real(8) :: :: vv(nstates), gvv(3,natoms,nstates)
real(8) :: :: dvec(3,natoms,nstates,nstates)
real(8) :: :: cc(nstates,nstates)
...
implicit none
real(8), parameter :: evtohartree=27.21139d0
real(8), parameter :: bohrtoang=0.52917721067d0
real(8) :: xx(3,7),X(7),Y(7),Z(7),scp(3),e1
integer :: i,j, nrap, ntap,dx1(7),dy1(7),dz1(7),dx(7),dy(7),dz(7)
integer :: igrad,lwork,info,repflag, ist, jst
real(8) :: dUx(7,3),dUy(7,3),dUz(7,3),uu(2,2),vv(2),tt(1),guu(3,7,2,2),gtt(3,7,1,1)
real(8) ::  gvv(3,7,2),dvec(3,7,2,2),eigvec(2,2)
...
do i=1,3
    do j=1,7
         tmpmat22(:,:) = guu(i,j,:,:)
         tmpmat22 = matmul(transpose(eigvec),matmul(tmpmat22,eigvec))
         do ist=1,2
          gvv(i,j,ist)=tmpmat22(ist,ist)
         enddo
         ist=1
         jst=2
         dvec(i,j,ist,jst)=tmpmat22(ist,jst)/(vv(jst)-vv(ist))
...
real(8) :: function dihedr1(i,j,k,l,X,Y,Z)
implicit real(8) ::(a-h,o-z)
real(8) :: tiny,pi
parameter(tiny=1.0d-13,pi=3.1415926535898d0)
real(8) :: X(*),Y(*),Z(*)
real(8) :: eij(3),ejk(3),ekl(3),u(3),v(3),w(3)
real(8) :: rij,rjk,rkl,aijk,ajkl,wejk
!   eij
! i-----j
!   aijk \ ejk
!         \ 
...
real(8) :: function bndlen(i,j,X,Y,Z)
implicit real(8) ::(a-h,o-z)
real(8) :: tiny
parameter(tiny=1.0d-13)
real(8) :: X(*),Y(*),Z(*)
bndlen=sqrt( (x(i)-x(j))*(x(i)-x(j)) + (y(i)-y(j))*(y(i)-y(j)) + (z(i)-z(j))*(z(i)-z(j)))
...
real(8) :: function bndang(i,j,k,X,Y,Z)
implicit real(8) ::(a-h,o-z)
real(8) :: X(*),Y(*),Z(*)
real(8) :: eji(3),ejk(3),rji,rjk
...
implicit none
real(8), intent(in)  :: x1(3), x1(3)
real(8), intent(out) :: x3(3)
...
subroutine convertit(X,Y,Z,dk,dx1,dy1,dz1)
      ...
      real(8) :: r1,t2
      real(8) :: qc(20)
      real(8) :: dx1(7),dy1(7),dz1(7)
      real(8) :: bndlen
      real(8) :: dngX(7),dngY(7),dngZ(7)
      real(8) :: dihedrIX,dihedrIY,dihedrIZ
      real(8) :: dihedrJX,dihedrJY,dihedrJZ
      real(8) :: dihedrKX,dihedrKY,dihedrKZ
      real(8) :: dihedrLX,dihedrLY,dihedrLZ
      real(8) :: dihedr2
      real(8) :: torH1,torH2,rH1HX,rH2HX
      real(8) :: ang1,ang2,ang3
      real(8) :: len1,len2,len3
...
implicit none
real(8), intent(in)  :: X(7),Y(7),Z(7)
integer, intent(in)  :: i,j,k
real(8), intent(out) :: dngX(7),dngY(7),dngZ(7)
real(8) :: ra,rb
real(8) :: to,be,go,cgo
real(8) :: bndlen
dngX    = 0d0
...
ra      = bndlen(j,i,X,Y,Z)
rb      = bndlen(j,k,X,Y,Z)
to      = (X(j)-X(i))*(X(j)-X(k))+(Y(j)-Y(i))*(Y(j)-Y(k))+(Z(j)-Z(i))*(Z(j)-Z(k))
be      = ((((X(j)-X(i))**2)+((Y(j)-Y(i))**2)+((Z(j)-Z(i))**2))**0.5d0) &
         *((((X(j)-X(k))**2)+((Y(j)-Y(k))**2)+((Z(j)-Z(k))**2))**0.5d0)
go      = to/be
cgo     = (-1d0/((1d0-go**2d0)**(1d0/2d0)))
dngX(j) = cgo*((2d0*X(j)-X(i)-X(k))*be-( (((X(j)-X(i))/ra)*rb) + (((X(j)-X(k))/rb)*ra) )*to)/(be**2d0)
dngY(j) = cgo*((2d0*Y(j)-Y(i)-Y(k))*be-( (((Y(j)-Y(i))/ra)*rb) + (((Y(j)-Y(k))/rb)*ra) )*to)/(be**2d0)
dngZ(j) = cgo*((2d0*Z(j)-Z(i)-Z(k))*be-( (((Z(j)-Z(i))/ra)*rb) + (((Z(j)-Z(k))/rb)*ra) )*to)/(be**2d0)
dngX(k) = cgo*( -1d0*(X(j)-X(i))*be-(((X(k)-X(j))/rb)*ra*to))/(be**2d0)
dngY(k) = cgo*( -1d0*(Y(j)-Y(i))*be-(((Y(k)-Y(j))/rb)*ra*to))/(be**2d0)
dngZ(k) = cgo*( -1d0*(Z(j)-Z(i))*be-(((Z(k)-Z(j))/rb)*ra*to))/(be**2d0)
dngX(i) = cgo*( -(X(j)-X(k))*be-(((X(i)-X(j))/ra)*rb*to))/(be**2d0)
dngY(i) = cgo*( -(Y(j)-Y(k))*be-(((Y(i)-Y(j))/ra)*rb*to))/(be**2d0)
dngZ(i) = cgo*( -(Z(j)-Z(k))*be-(((Z(i)-Z(j))/ra)*rb*to))/(be**2d0)
end subroutine angder
...
real(8) :: qs(nqs)
real(8) :: qb(nqb)
real(8) :: sqb(nqb),cqb(nqb)
real(8) :: qc(20),dut(20)
real(8) :: v0(nrap,ntap)
real(8) :: PI
real(8) :: dnc1,dnc2
real(8) :: dch1,dch2
real(8) :: KNHB1(nrap,ntap),KNHB2(nrap,ntap)
real(8) :: KNC1(nrap,ntap),KNC2(nrap,ntap)
real(8) :: KCH1(nrap,ntap),KCH2(nrap,ntap)
real(8) :: KCH3(nrap,ntap)
real(8) :: KCNHA1(nrap,ntap),KCNHA2(nrap,ntap)
real(8) :: KCNHB1(nrap,ntap),KCNHB2(nrap,ntap)
real(8) :: KNCH1(nrap,ntap),KNCH2(nrap,ntap)
real(8) :: JNHB1(nrap,ntap),JNHB2(nrap,ntap)
real(8) :: JNHB3(nrap,ntap)
real(8) :: JNC1(nrap,ntap),JNC2(nrap,ntap)
real(8) :: JNC3(nrap,ntap)
real(8) :: JCNHA1(nrap,ntap),JCNHA2(nrap,ntap)
real(8) :: JCNHA3(nrap,ntap)
real(8) :: JCNHB1(nrap,ntap),JCNHB2(nrap,ntap)
real(8) :: JCNHB3(nrap,ntap)
real(8) :: GNHB1(nrap,ntap),GNHB2(nrap,ntap)
real(8) :: GNHB3(nrap,ntap)
real(8) :: GNC1(nrap,ntap),GNC2(nrap,ntap)
real(8) :: GNC3(nrap,ntap)
real(8) :: v3,v4
real(8) :: torH1,torH2,rH1HX,rH2HX
real(8) :: ang1,ang2,ang3
real(8) :: len1,len2,len3
real(8) :: DD1,DD2,RR,ft2,fa2
real(8) :: ft1(nrap,ntap)
real(8) :: fa1(nrap,ntap)
PI=4.D0*DATAN(1.D0)
ut=0d0
vs=0d0
vb=0d0
...
! STRETCH 1 (NHB)
!       if (ii .eq. 1) then
        vs=GNHB1(ra1,ra2) *(1d0-exp(GNHB2(ra1,ra2)*(qc(1)-GNHB3(ra1,ra2))))**2d0
        vs=vs+KNHB1(ra1,ra2)*(qc(1)-KNHB2(ra1,ra2))**4d0
! STRETCH 2 (NC)
        vs=vs+GNC1(ra1,ra2)*(1d0-exp(GNC2(ra1,ra2)*(qc(2)-GNC3(ra1,ra2))))**2d0
        vs=vs+KNC1(ra1,ra2)*(qc(2)-KNC2(ra1,ra2))**4d0
! STRETCHES 3-5 (CH)
        do i=3,nqs
        vs=vs+KCH1(ra1,ra2)*(1d0-exp(KCH2(ra1,ra2)*(qc(i)-KCH3(ra1,ra2))))**2d0
        enddo
! BENDS
! BEND 1 (CNHA)
...
! BEND 3-5 (NCH)
      ...
      ft2=-3.0d0
      DD1=2.2d0
      v3=ft1(ra1,ra2)*(1d0-(4d0*(torH2**3d0)-3d0*torH2))* exp(((rH2HX-DD1)**2d0)/ft2)
      DD2=1.881d0
      RR=1.761d0
      fa2=-2.0d0
...
! STRETCHES
! STRETCH 1 (NHB)
        vs=JNHB1(ra1,ra2)*exp(JNHB2(ra1,ra2)*(qc(1)-JNHB3(ra1,ra2))**2d0)
! STRETCH 2 (NC)
        vs=vs+JNC1(ra1,ra2)*exp(JNC2(ra1,ra2)*(qc(2)-JNC3(ra1,ra2))**2d0)
! STRETCHES 3-5 (CH)
! BENDS
! BEND 1 (CNHA)
vb=JCNHA1(ra1,ra2)*exp(JCNHA2(ra1,ra2)*(cqb(1)-JCNHA3(ra1,ra2))**2d0)
! BEND 2 (CNHB)
vb=vb+JCNHB1(ra1,ra2)*exp(JCNHB2(ra1,ra2)*(cqb(2)-JCNHB3(ra1,ra2))**2d0)
! BEND 3-5 (NCH)
endif
! COMBINE
ut=vs+vb+u0+v3+v4
...
! STRETCH 1 (NHB)
!       if (ii .eq. 1) then
dut(1)=-2d0*GNHB2(ra1,ra2)*GNHB1(ra1,ra2)*exp(GNHB2(ra1,ra2)*(qc(1)-GNHB3(ra1,ra2)))*(1d0-exp(GNHB2(ra1,ra2)*(qc(1)-GNHB3(ra1,ra2))))
...
! BENDS
! BEND 1 (CNHA)
dut(6)=KCNHA1(ra1,ra2)*(cqb(1)-KCNHA2(ra1,ra2))*2d0*(-sqb(1))
! BEND 2 (CNHB)
dut(7)=KCNHB1(ra1,ra2)*(cqb(2)-KCNHB2(ra1,ra2))*2d0*(-sqb(2))
! BEND 3-5 (NCH)
        do i=3,nqb
...
!
      real(8) :: GNC1(nrap,ntap),GNC2(nrap,ntap)
      real(8) :: GNC3(nrap,ntap)
      real(8) :: ft1(nrap,ntap),fa1(nrap,ntap)
...
!***********************************************************************
      ...
      real(8) :: xyz(4,3)
      real(8) :: dx(22)
      real(8) :: rap(nrap,3),tap(ntap,3)
      integer i,ii,j
      integer fg(2)
      real(8) :: u,v,f(4)
      real(8) :: bndlen,bndang
      real(8) :: H(4),EN
      real(8) :: dH(4,20),dHe(20)
      real(8) :: df(4,2)
      real(8) :: dTr(2)
      real(8) :: dx1(7),dy1(7),dz1(7)
      real(8) :: beta
...
!***********************************************************************
      ...
      real(8) :: tap(ntap,3)
      real(8) :: f(4)
      real(8) :: un,up,vn,vp,dnom
      real(8) :: df(4,2)
      real(8) :: dun,dup,dvn,dvp,ddnom(2)
      integer fg(2)
...
! gives the primary coordinate contribution
      ...
      real(8) :: a12(4)
      real(8) :: const(4)
      real(8) :: ga1(4),ga2(4),ga3(4)
      real(8) :: gb1(4),gb2(4),gb3(4)
      real(8) :: ka1(4),ka2(4)
      real(8) :: c(0:10)
      real(8) :: re,de,rmin,pi,rx
      real(8) :: fy,dfdr,u,dydr,lj,dljdr
      real(8) :: rr,tt,dv2dr,dv2dt,v3
      integer ri,i,igrad
      real(8) :: X(7),Y(7),Z(7)
      real(8) :: dx(7),dy(7),dz(7)
      real(8) :: ae,e1,e2,e3,f1,f2,f3
      real(8) :: g1,g2,g3,g4,i2,i3,i4
      real(8) :: h2,h3,h4,j2,j3,j4
      real(8) :: n1,n2,n3,n4
      real(8) :: bndlen
      real(8) :: dihedr2
      real(8) :: ak,tt0,gr1,gr2,gr3
      real(8) :: fr1(5),fr2(5),fr3(5)
      real(8) :: ft2(5),ft3(5)
      real(8) :: dihedrIX,dihedrIY,dihedrIZ
      real(8) :: dihedrJX,dihedrJY,dihedrJZ
      real(8) :: dihedrKX,dihedrKY,dihedrKZ
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     & *(ka1(i)*(rr-ka2(i))*2d0)
      ...
      real(8) :: function dihedr2(i,j,k,l,X,Y,Z)
!************************************************************************
! Evaluate the dihedral angle between atoms i, j, k, and l
! Input: i,j,k,l (indices of 4 atoms)
!        i--j--k--l
...
!************************************************************************
      ...
      real(8) :: eij(3),ejk(3),ekl(3),u(3),v(3),w(3)
      real(8) :: rij,rjk,rkl,aijk,ajkl,wejk
      real(8) :: kkX,kkY,kkZ,yyX,yyY,yyZ
      real(8) :: llX,llY,llZ,eeX,eeY,eeZ
      real(8) :: oo1(3),oo2(3),so1(3),so2(3)
      real(8) :: sso1,sso2,rso1,rso2
      real(8) :: ab(3)
      real(8) :: sab,p1p2,ra
...
!--------------------------------------------------!
      ...
      real(8) :: function dihedrIX(i,j,k,l,X,Y,Z)
!************************************************************************
...
!************************************************************************
      ...
      real(8) :: eij(3),ejk(3),ekl(3),u(3),v(3),w(3)
      real(8) :: rij,rjk,rkl,aijk,ajkl,wejk
      real(8) :: ijX,ijY,ijZ,kjX,kjY,kjZ
      real(8) :: lkX,lkY,lkZ,jkX,jkY,jkZ
      real(8) :: oo1(3),oo2(3),so1(3),so2(3)
      real(8) :: sso1,sso2,rso1,rso2
      real(8) :: ab(3)
      real(8) :: sab,p1p2,ra
      real(8) :: ld,rd,td,bd,id,fd
...
!------------------Xi---------------------------
      ...
      real(8) :: function dihedrIY(i,j,k,l,X,Y,Z)
!************************************************************************
! Converts derivative from internal to XYZ for
! dihedral angle between atoms i, j, k, and l
! Input: i,j,k,l (indices of 4 atoms)
...
!------------------Yi---------------------------
      ...
      real(8) :: function dihedrIZ(i,j,k,l,X,Y,Z)
!************************************************************************
! Converts derivative from internal to XYZ for
! dihedral angle between atoms i, j, k, and l
! Input: i,j,k,l (indices of 4 atoms)
...
!------------------Zi---------------------------
      ...
      real(8) :: function dihedrLX(i,j,k,l,X,Y,Z)
!************************************************************************
! Converts derivative from internal to XYZ for
! dihedral angle between atoms i, j, k, and l
! Input: i,j,k,l (indices of 4 atoms)
...
!------------------Xl---------------------------
      ...
      real(8) :: function dihedrLY(i,j,k,l,X,Y,Z)
!************************************************************************
! Converts derivative from internal to XYZ for
! dihedral angle between atoms i, j, k, and l
! Input: i,j,k,l (indices of 4 atoms)
...
!------------------Yl---------------------------
      ...
      real(8) :: function dihedrLZ(i,j,k,l,X,Y,Z)
!************************************************************************
! Converts derivative from internal to XYZ for
! dihedral angle between atoms i, j, k, and l
! Input: i,j,k,l (indices of 4 atoms)
...
!------------------Zl---------------------------
      ...
      real(8) :: function dihedrJX(i,j,k,l,X,Y,Z)
!************************************************************************
! Converts derivative from internal to XYZ for
! dihedral angle between atoms i, j, k, and l
! Input: i,j,k,l (indices of 4 atoms)
...
!------------------Xj---------------------------
      ...
      real(8) :: function dihedrJY(i,j,k,l,X,Y,Z)
!************************************************************************
! Converts derivative from internal to XYZ for
! dihedral angle between atoms i, j, k, and l
! Input: i,j,k,l (indices of 4 atoms)
...
! Input: i,j,k,l (indices of 4 atoms)
!        i--j--k--l
      real(8) :: function dihedrJZ(i,j,k,l,X,Y,Z)
      implicit real(8) ::(a-h,o-z)
      real(8) :: tiny,pi
      parameter(tiny=1.0d-13,pi=3.1415926535898d0)
      real(8) :: X(*),Y(*),Z(*)
...
! Input: i,j,k,l (indices of 4 atoms)
!        i--j--k--l
real(8) :: function dihedrKX(i,j,k,l,X,Y,Z)
implicit real(8) ::(a-h,o-z)
real(8) :: tiny,pi
parameter(tiny=1.0d-13,pi=3.1415926535898d0)
real(8) :: X(*),Y(*),Z(*)
...
rd=2d0*(X(k)-X(j))*(lkZ**2+lkY**2)+2d0*(X(k)-X(l))*(jkZ**2+jkY**2) &
     &   -2*(2d0*X(k)-X(j)-X(l))*(lkZ*jkZ+lkY*jkY)
bd=((ld*sso2)+(sso1*rd))/(2d0*p1p2)
!ra=sab/p1p2
id=((td*p1p2)-(bd*sab))/(p1p2**2d0)
...
! Input: i,j,k,l (indices of 4 atoms)
!        i--j--k--l
real(8) :: function dihedrKY(i,j,k,l,X,Y,Z)
implicit real(8) ::(a-h,o-z)
real(8) :: tiny,pi
parameter(tiny=1.0d-13,pi=3.1415926535898d0)
real(8) :: X(*),Y(*),Z(*)
...
real(8) :: function dihedrKZ(i,j,k,l,X,Y,Z)
! Converts derivative from internal to XYZ for
! dihedral angle between atoms i, j, k, and l
! Input: i,j,k,l (indices of 4 atoms)
!         i--j--k--l
...
td=(-2d0*Z(k)+2d0*Z(j))*ijY*lkY-(-2d0*Z(k)+Z(j)+Z(l))*ijY*jkY+ijZ*kjY*lkY-ijZ*kjY*jkY &
    -ijZ*kjX*jkX+ijZ*kjX*lkX-(-2d0*Z(k)+Z(j)+Z(l))*ijX*jkX+(-2d0*Z(k)+2d0*Z(j))*ijX*lkX
! deriv of sso1 for Zk
ld=2d0*(Z(k)-Z(j))*(ijY**2+ijX**2)-2*(ijZ*ijY*kjY+ijZ*ijX*kjX)
! deriv of sso2 for Zk
rd=2d0*(Z(k)-Z(j))*(lkY**2+lkX**2)+2d0*(Z(k)-Z(l))*(jkY**2+jkX**2)-2*(2d0*Z(k)-Z(j)-Z(l))*(lkY*jkY+lkX*jkX)
bd=((ld*sso2)+(sso1*rd))/(2d0*p1p2)
!ra=sab/p1p2
id=((td*p1p2)-(bd*sab))/(p1p2**2d0)
